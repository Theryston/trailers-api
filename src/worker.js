...
export default async function worker({
  ...
  try {
    await processLog({
      ...
      description: "Process was started",
    });
    ...
    await processLog({
      id: processId,
      status: PROCESS_STATUS.FINDING_TRAILER_PAGE,
      description: `Looking for trailer on: ${services
        ...
        (async () => {
          ...
          const serviceTrailers = await service.func({
            ...
            onTrailerFound: async (foundTrailerPage) => {
              processLog({
                id: processId,
                status: PROCESS_STATUS.TRYING_TO_DOWNLOAD,
                description: `Trying to download the trailers from: ${service.name}`,
              });
    ...
    const servicesResultsWithTrailers = servicesResults.filter(
      (serviceResult) =>
    ...
    if (!servicesResultsWithTrailers.length) {
      await processLog({
        id: processId,
        status: PROCESS_STATUS.NO_TRAILERS,
        description:
          "Trailers not found. Try again with another title variation",
      });
      return;
    }

    const sortedServicesResults = servicesResultsWithTrailers.sort(
      (a, b) => b.serviceResult.length - a.serviceResult.length
    );
    const bestServiceResult = getBestServiceResult(
      sortedServicesResults,
      lang,
      processId
    );
    ...
    await processLog({
      id: processId,
      status: PROCESS_STATUS.FOUND_TRAILER,
      description: `Found the best trailer on ${bestServiceResult.serviceResult.name}`,
      trailerPage: bestServiceResult.trailerPage,
    });

    await processLog({
      id: processId,
      status: PROCESS_STATUS.SAVING,
      description: `Saving videos: ${trailers
        .map((trailer) => trailer.title)
        .join(", ")}`,
    });
    ...
    for (const trailer of trailers) {
      ...
      const url = await uploadFile(trailer.path);
      ...
      for (const subtitle of trailer.subtitles) {
        const subtitleUrl = await uploadFile(subtitle.path);

        await db.insert(subtitlesSchema).values({
  ...
  return await new Promise((resolve, reject) => {
    ffmpeg.ffprobe(videoPath, (err, data) => {
      if (err) {
        ...
        const audios = data.streams.filter(
...
function getBestServiceResult(servicesResults, lang, processId) {
  let best = servicesResults.find((serviceResult) =>
    serviceResult.serviceResult.every(
      (t) =>
        t.subtitles.find((sub) => compareLang(sub.language, lang)) &&
        t.langs.find((l) => compareLang(l, lang))
    )
  );

  if (!best) {
    best = servicesResults.find((serviceResult) =>
      serviceResult.serviceResult.every((t) =>
        t.langs.find((l) => compareLang(l, lang))
      )
    );
  }
  ...
  if (!best) {
    best = servicesResults.find((serviceResult) =>
      serviceResult.serviceResult.find((t) =>
        t.langs.find((l) => compareLang(l, lang))
      )
    );
  }
