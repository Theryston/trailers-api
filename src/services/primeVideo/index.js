...
export default async function primeVideo({
  ...
  try {
    if (!trailerPage) {
      ...
      const program = googleResults.find((result) => {
        ...
        return (
          ...
          result.link.startsWith("https://www.primevideo.com")
        );
      });
    ...
    const { data: trailerInfo } = await axios.get(
    ...
    let audioTrack = playbackUrls.audioTracks.find((aT) =>
      compareLang(aT.languageCode, lang)
    );
    ...
    const subtitleUrls = trailerInfo.subtitleUrls.filter(
      (sU) => sU.trackGroupId === trackGroupId
    );
    ...
    let urlSet =
      playbackUrls.urlSets[
        Object.keys(playbackUrls.urlSets).find((key) => {
          const currentUrlSet = playbackUrls.urlSets[key];
          return (
            (currentUrlSet.urls.manifest.audioTrackId ===
              audioTrack.audioTrackId ||
              currentUrlSet.urls.manifest.audioTrackId === "ALL" ||
              !currentUrlSet.urls.manifest.audioTrackId) &&
            currentUrlSet.urls.manifest.videoQuality === "HD"
          );
        })
      ];
    ...
    if (!urlSet) {
      urlSet =
        playbackUrls.urlSets[
          Object.keys(playbackUrls.urlSets).find((key) => {
            const currentUrlSet = playbackUrls.urlSets[key];
            return currentUrlSet.urls.manifest.videoQuality === "HD";
          })
        ];
    }

    if (!urlSet) {
      urlSet = playbackUrls.urlSets[Object.keys(playbackUrls.urlSets)[0]];
    }
    ...
    const representations = $("Representation")
      ...
      .map((representation) => {
        ...
        if (representation.children.find((child) => child.name === "baseurl")) {
          baseUrl = representation.children.find(
            (child) => child.name === "baseurl"
          ).children[0].data;
        } else {
          baseUrl = representation.children[0].children.find(
            (child) => child.name === "baseurl"
          ).children[0].data;
        }
    ...
    const videos = representations.filter((representation) =>
      representation.baseUrl?.includes("video")
    );
    const audios = representations.filter((representation) =>
      representation.baseUrl?.includes("audio")
    );
    ...
    if (fullAudioTracks || !audioTrack) {
      audiosPath.push(...audios.map((r) => r.baseUrl));
    } else {
      audiosPath.push(
        audios.find(
          (r) =>
            r.adaptationSet.attribs.audiotrackid === audioTrack.audioTrackId
        )?.baseUrl
      );
    }
    ...
    const tempDir = fs.mkdtempSync(
      path.join(GLOBAL_TEMP_FOLDER, "prime-video-")
    );
